...
with torch.profiler.profile(
    ...
    schedule=torch.profiler.schedule(wait=0, warmup=1, active=len(train_loader)), # need to be set acc to batch per epoch
    ...
    with_flops=True,
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, batch in enumerate(tqdm(train_loader)):
                ...
                with torch.no_grad():
                    flop_analyzer = FlopCountAnalysis(model, (x, y))
                batch_flops = flop_analyzer.total()
                epoch_flops_accum += batch_flops  
                
                # Log per-batch metrics
                print(
                    f"Epoch {epoch}, Batch {batch_idx} | Time: {batch_time:.4f}s | "
                    f"Tokens: {tokens_in_batch} | FLOPs: {batch_flops} | "
            ...
            flops_in_tera = epoch_flops_accum / 1e12
