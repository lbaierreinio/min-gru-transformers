...
class TransformerMinGRUEncoderBlock(nn.Module):
  def __init__(self, num_heads, num_hiddens, ffn_num_hiddens, chunk_size, dropout, bias=False):
    super().__init__()
  ...
  def forward(self, x):
    ...
    x_out = x_out.reshape(num_chunks, batch_size, self.chunk_size, num_hiddens) # (N, B, C, H)
    x_out = x_out.transpose(1, 0) # (B, N, C, H)
    x_out = x_out.reshape(batch_size, -1, num_hiddens) # (B, S, H)
    
    return x_out + self.bi_min_gru(self.ln(x_out)) # Skip connection & layer_norm around BiMinGRU
